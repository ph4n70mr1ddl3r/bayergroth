#ifndef BAYER_GROTH_COMPLETE_H
#define BAYER_GROTH_COMPLETE_H
...
struct ShuffleProof {
    std::vector<std::vector<mpz_class>> A;  // g^S
    std::vector<std::vector<mpz_class>> B;  // h^S
    std::vector<std::vector<mpz_class>> D;  // Pedersen commitments g^α h^β

    std::vector<mpz_class> z1;   // shuffle randomness response
    std::vector<mpz_class> z5;   // row alpha response
    std::vector<mpz_class> z6;   // row beta response
    std::vector<mpz_class> z7;   // column alpha response
    std::vector<mpz_class> z8;   // column beta response
    mpz_class z9;                // total alpha + c * sum(S)
    mpz_class z10;               // total beta + c * sum(S)

    mpz_class t;                 // product commitment for a
    mpz_class u;                 // product commitment for b
    mpz_class d_perm;            // permutation commitment product
};

class BayerGrothComplete {
public:
    BayerGrothComplete(int securityBits = 256);
    ~BayerGrothComplete();

    KeyPair generateKeyPair();
    Ciphertext encrypt(const PublicKey& pk, const mpz_class& message, const mpz_class& randomness);
    Ciphertext decrypt(const PublicKey& pk, const mpz_class& sk, const Ciphertext& ct);
...
private:
    ...
    mpz_class discreteLog(const mpz_class& base, const mpz_class& target, const mpz_class& mod);
};
